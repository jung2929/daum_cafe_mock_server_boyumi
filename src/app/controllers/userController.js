  ...
  if (!token) {
    ...
    if (!device)
      return res.json({
        isSuccess: false,
        code: 304,
        message: 'device token 정보를 입력 해주세요.',
      })
    ...
    try {
      ...
      try {
        ...
        // 디바이스 토큰 있는지 없는지
        const selectDeviceInfoQuery = `SELECT phone
        FROM phone_info WHERE userId=?;
        `
        const [ selectDeviceRows ] = await connection.query(selectDeviceInfoQuery, selectUserInfoParams)

        console.log('select Device INfo : ' + selectDeviceRows)
        if (selectDeviceRows.length === 0) {
          console.log('check1')
          const insertDeviceInfoQuery = `INSERT INTO phone_info (userId, phone) VALUES (?,?);
                    `
          const [ deviceInfoRows ] = await connection.query(insertDeviceInfoQuery, [ selectUserInfoParams, device ])
          console.log(deviceInfoRows)
        } else if (device != selectDeviceRows.phone) {
          const patchDeviceQuery = `UPDATE phone_info SET phone=? WHERE userId=?;`
          const [ deviceInfoRows ] = await connection.query(patchDeviceQuery, [ device, selectUserInfoParams ])
        }
